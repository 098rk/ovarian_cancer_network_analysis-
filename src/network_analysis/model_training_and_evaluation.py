...
class OvarianCancerNetworkProcessor:
    ...
    def build_complete_network(self) -> nx.DiGraph:
        ...
        logger.info(f"Complete network built with {self.G.number_of_nodes()} nodes and {self.G.number_of_edges()} edges")
    ...
    def _classify_node_type(self, node_name: str) -> str:
        ...
        if any(x in node_lower for x in ['mrna', 'mirna', 'pre-mirna']):
            return 'RNA'
        elif any(x in node_lower for x in ['p53', 'nfkb', 'creb', 'bax', 'akt', 'mdm2', 'ikk', 'chk2', 'atm', 'wip1', 'ksrp', 'tnfr', 'mrn']):
            return 'Protein'
    ...
    def extract_comprehensive_features(self, pagerank_scores: Dict[str, float]):
        ...
        ] + ['type_' + t for t in ['Protein', 'RNA', 'Cellular_Component', 'Phenotype', 'Stimulus', 'Other']]
        ...
        for node in node_list:
            ...
            try:
                clustering = nx.clustering(self.G.to_undirected(), node)
            except:
                clustering = 0
    ...
    def _calculate_regulatory_potential(self, node: str) -> float:
        ...
        out_degree = self.G.out_degree(node)
        max_out_degree = max([d for n, d in self.G.out_degree()]) if self.G.nodes() else 1
    ...
    def generate_comprehensive_sequences(self, sequence_length: int = 10, walks_per_node: int = 5) -> Tuple[np.ndarray, np.ndarray]:
        """Generate comprehensive sequences using biased random walks"""
        logger.info(f"Generating comprehensive sequences (length: {sequence_length}, walks per node: {walks_per_node})...")
        ...
        target_values = np.array([pagerank_scores[node] for node in node_list]).reshape(-1, 1)
        if len(target_values) > 0:
            target_values = self.target_scaler.fit_transform(target_values).flatten()
        else:
            target_values = np.array([])
        
        target_dict = {node: target_values[i] for i, node in enumerate(node_list)} if len(target_values) > 0 else {}
    ...
    def _biased_random_walk(self, start_node: str, length: int) -> List[str]:
        ...
        for step in range(length - 1):
            ...
            if not neighbors:
                # If no neighbors, use restart or previous node
...
class ComprehensiveRCNNTrainer:
    ...
    def __init__(self, model, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=8, factor=0.5)
    ...
    def train(self, train_loader, val_loader, epochs=100):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                ...
                logger.info(f'Epoch {epoch:3d}: Train Loss: {train_loss:.6f}, Val Loss: {val_loss:.6f}, LR: {current_lr:.6f}')
        ...
        # Load best model
        try:
            self.model.load_state_dict(torch.load('best_comprehensive_rcnn_model.pth'))
        except:
...
def plot_comprehensive_results(train_losses, val_losses, evaluation, processor):
    ...
    for feature in processor.feature_names:
        ...
        for node in processor.G.nodes():
            try:
                feature_values.append(processor.node_features[node][processor.feature_names.index(feature)])
